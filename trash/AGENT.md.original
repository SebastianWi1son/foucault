...
- **CODE.md 代码块格式（2026-09-13 用户定案）**：CODE.md 里的代码块是**改动定位图，不是粘贴源**。规则：
  ① **删除文件中已有的注释**（用户文件里本来就有，不必重复展示）
  ② **只在本批次要修改/新增的地方写注释**——注释的作用是**标记改动位置**，不是解释代码
  ③ 改动处用醒目标记（如 `// ★4a-5 新增` / `// ★4a-5 改动：原 xxx`），一眼可扫
  → 目的：用户**逐段替换**而非整文件覆盖（避免丢掉原有注释）。施工稿须显式写清「哪几行要替换成什么」
