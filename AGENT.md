...
⑤ 成员命名（F12）：**统一尾下划线 `_` 后缀**（用户定案 2026-08-30；`x_`/`q0_`/`e_int_`，区分成员与局部/参数）
...
- **文档 = 已决断内容的归档，不是讨论载体**：日常问答/学习/思路讨论直接在对话中完成，**不因每次答疑而写文档**；只有结论升级为决策（选型、设计定案、规则确立）时才归档进对应大类文档
- `docs/` 按角色分工：**DESIGN.md（设计权威）** / RESEARCH.md（调研） / DEV.md（开发流程） / 复盘.md（错误清单） / 待办.md（悬挂待办）
